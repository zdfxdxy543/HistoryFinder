...
class EventRuleRegistry:
    ...
    def _register_defaults(self):
        ...
        self.rules["literary_work"] = EventRule(
            ...
            possible_outcomes=[
                PO("epic", severity=0.20, base_weight=0.25,
                   title_template_cn="$SETTLEMENT_NAME出现新的长篇叙事诗",
                   effects=[
                       ModifyTreasury("$SETTLEMENT", delta=-18.0,
                                      reason="literary_patronage"),
                       ModifyCulturalInfluence("$SETTLEMENT", delta=0.35,
                                               reason="epic_poem"),
                       ModifyLegitimacy("$SETTLEMENT", delta=0.015,
                                        reason="shared_identity"),
                   ]),
                PO("drama", severity=0.18, base_weight=0.25,
                   title_template_cn="$SETTLEMENT_NAME出现新的剧作",
                   effects=[
                       ModifyTreasury("$SETTLEMENT", delta=-15.0,
                                      reason="literary_patronage"),
                       ModifyCulturalInfluence("$SETTLEMENT", delta=0.28,
                                               reason="public_drama"),
                       ModifyStability("$SETTLEMENT", delta=0.015,
                                       reason="public_performance"),
                   ]),
                PO("chronicle", severity=0.22, base_weight=0.25,
                ...
                PO("lyric_cycle", severity=0.16, base_weight=0.25,
                   title_template_cn="$SETTLEMENT_NAME流传新的组诗",
                   effects=[
                       ModifyTreasury("$SETTLEMENT", delta=-12.0,
                                      reason="literary_patronage"),
                       ModifyCulturalInfluence("$SETTLEMENT", delta=0.25,
                                               reason="lyric_cycle"),
                       ModifyStability("$SETTLEMENT", delta=0.01,
                                       reason="shared_recitation"),
                   ]),
            ],
